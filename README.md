...
> 这是一个“智力碾压式”六级复盘与输出训练 Skill：把题目拆成证据链和命题机制，也把作文、翻译做成“有限提示 → 独立初稿 → 证据评分 → 用户二稿 → 换题迁移”的闭环。它支持翻译回译三层诊断和人工阅卷证据审计，不包含听力模块，也不把推断、第三方经验或模型分数冒充官方规则与正式成绩。

面向 Codex 的大学英语六级深度复盘与输出训练 Skill。它不是商业产品，也不承诺替代教师或官方阅卷；重点是把阅读题、作文和翻译变成可复核、能自行修改、能跨题迁移的训练过程。
...
- **作文·训练**：用户可选无提示、轻提示或教学提示；默认只给少量高价值表达和可变句型骨架，随后由用户完成初稿、二稿和换题迁移。完整范文不会抢在用户作答之前出现。
- **翻译·回译训练**：把中文信息单元、英文回译后的意义差异和英文自身自然度分开检查；回译只证明信息是否大致保真，不能掩盖中式英语或错误搭配。
- **人工阅卷·证据审计**：以官方大纲与评分样卷为根锚点，将可核验 CET-6 人评、其他 EFL 人评研究、机构经验和匿名转载分级；没有官方发布链的“新标准、权重和固定扣分表”不会进入评分规则。
